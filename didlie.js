didlie.qrCode = {
	...
	createCanvas: function (text, sizeMultiplier) {
		...
		var typeNumber = didlie.qrCode.getTypeNumber(text);
		...
		for (var key in keyValuePair) {
			...
			try {
				if (document.getElementById(key)) {
					...
					document.getElementById(key).appendChild(didlie.qrCode.createCanvas(value, sizeMultiplier));
				}
			}
			catch (e) {
				// for browsers that do not support canvas (IE8)
				document.getElementById(key).innerHTML = didlie.qrCode.createTableHtml(value);
			}
		}
	}
};
